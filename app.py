...
# Source: Kaggle Big Data Bowl 2025 + OpenWeather adjustments
# Build vA57 | Vishvin Ramesh
...
st.caption("Build vA57 | by Vishvin Ramesh")
...
def safe_read_file(file_like):
    """Reads CSV/ZIP/Excel with auto-detected encoding and graceful fallbacks."""
    try:
            ...
            if zipfile.is_zipfile(file_like):
                with zipfile.ZipFile(file_like, "r") as z:
                    ...
                    with z.open(csv_name) as f:
                        raw = f.read(4096)
                        enc2 = chardet.detect(raw).get("encoding", "utf-8")
                        f = io.BytesIO(raw + f.read())
                        f.seek(0)
                        return pd.read_csv(f, encoding=enc2)
            file_like.seek(0)
            try:
...
def load_data(uploaded_file=None):
    """Loads dataset from uploaded or local file."""
    try:
        if uploaded_file:
...
st.caption("Data: Kaggle Big Data Bowl 2025 | Build vA57 | by Vishvin Ramesh")
